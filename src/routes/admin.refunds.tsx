  ...
  head: () => ({
    meta: [
      { title: "Remboursements — Administration Diambar Agro" },
      {
        name: "description",
        content:
          "Validez, rejetez et exécutez les remboursements issus des litiges, retours et incidents de course.",
      },
      { property: "og:title", content: "Remboursements — Administration" },
      {
        property: "og:description",
        content: "Outil de remboursement de la plateforme Diambar Agro.",
      },
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="glass rounded-2xl p-4">
          ...
          <div className="text-xs text-muted-foreground">{formatFCFA(pendingTotal)}</div>
        </div>
        <div className="glass rounded-2xl p-4">
          <div className="text-xs text-muted-foreground">Remboursé (cumul)</div>
          <div className="mt-1 text-2xl font-bold">{formatFCFA(paidTotal)}</div>
        </div>
        <div className="glass rounded-2xl p-4">
          <div className="text-xs text-muted-foreground">Dossiers traités</div>
          <div className="mt-1 text-2xl font-bold">
            {refunds.filter((r) => r.status !== "pending").length}/{refunds.length}
          </div>
        </div>
      </div>
              ...
              <div className="flex flex-wrap gap-2">
                {METHODS.map((m) => (
                  <button
                    ...
                    className={`rounded-xl border px-3 py-1.5 text-xs font-medium transition ${form.method === m ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:bg-accent"}`}
                  >
                    {m}
                  </button>
                ))}
