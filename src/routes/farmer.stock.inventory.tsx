...
function InventoryPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Inventaire"
        subtitle="Comparez le stock théorique au stock réel"
        actions={
            ...
            {items.map((p) => {
              ...
              return (
                <TableRow key={p.id}>
                  ...
                  <TableCell
                    className={`text-right font-semibold ${diff === 0 ? "text-muted-foreground" : diff > 0 ? "text-emerald-500" : "text-rose-500"}`}
                  >
                    {diff > 0 ? "+" : ""}
                    {diff}
                  </TableCell>
