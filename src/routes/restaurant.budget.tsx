...
export const Route = createFileRoute("/restaurant/budget")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Pilotez votre budget mensuel d'approvisionnement : dépenses par catégorie, par fournisseur et alertes de dépassement." },
    { property: "og:title", content: "Budget d'achat — Espace restaurant" },
    { property: "og:description", content: "Budget mensuel, dépenses par catégorie et alertes de dépassement." },
  ...
  const exportCsv = () => {
    downloadCsv(
      ...
      budget.categories.map((c) => {
        ...
        return [c.label, c.allocated, s, c.allocated - s, c.allocated ? Math.round((s / c.allocated) * 100) : 0];
      }),
    );
  };
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            <Button variant="outline" className="gap-2" onClick={exportCsv}><Download className="h-4 w-4" />Exporter CSV</Button>
            <Button asChild className="gap-2"><Link to="/restaurant/marketplace">Commander</Link></Button>
          </div>
        }
      />
      ...
      {overThreshold && (
        <div className="flex items-start gap-3 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm">
          ...
          <div>
            <div className="font-semibold text-amber-700 dark:text-amber-300">Seuil d'alerte atteint ({usage}% du budget)</div>
            <p className="text-muted-foreground mt-0.5">Vous approchez de la limite mensuelle. Ajustez vos commandes récurrentes ou relevez le budget.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <KpiCard label="Reste disponible" value={formatFCFA(Math.max(0, remaining))} icon={TrendingDown} />
      ...
      <div className="glass rounded-2xl p-5 space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <span className="text-xs text-muted-foreground">Seuil d'alerte : {budget.alertThreshold}%</span>
        </div>
            ...
            return (
              <div key={c.key} className="space-y-1.5">
                <div className="flex items-center justify-between gap-3 text-sm">
                  ...
                  <span className="text-muted-foreground text-xs">{formatFCFA(s)} / {formatFCFA(c.allocated)}</span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    onBlur={(e) => { budgetActions.setCategory(c.key, Number(e.target.value) || 0); toast.success(`Enveloppe « ${c.label} » mise à jour`); }}
                    className="h-8 max-w-36 text-xs"
                  />
                  <span className="text-[11px] text-muted-foreground">enveloppe allouée (FCFA)</span>
                </div>
              </div>
            );
          })}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="h-60 mt-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chart}>
                ...
                <Bar dataKey="Budget" fill="hsl(var(--muted-foreground))" radius={[6, 6, 0, 0]} opacity={0.35} />
        ...
        <div className="glass rounded-2xl p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input type="number" value={draftMonthly} onChange={(e) => setDraftMonthly(e.target.value)} />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs text-muted-foreground">Seuil d'alerte (%)</label>
            <Input type="number" value={draftThreshold} onChange={(e) => setDraftThreshold(e.target.value)} />
          </div>
          <Button className="w-full gap-2" onClick={saveBudget}><Save className="h-4 w-4" />Enregistrer</Button>
          <Button variant="ghost" className="w-full" onClick={() => { budgetActions.reset(); setDraftMonthly("1200000"); setDraftThreshold("80"); toast.success("Budget réinitialisé"); }}>Réinitialiser</Button>
        </div>
      </div>

      <div className="glass rounded-2xl overflow-hidden">
        <div className="px-4 py-3 border-b border-border font-semibold text-sm">Dépenses par fournisseur</div>
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-xs text-muted-foreground">
            <tr>
              ...
              <th className="text-right font-medium px-4 py-3 hidden sm:table-cell">Part du budget</th>
            </tr>
          </thead>
              ...
              return (
                <tr key={s.id} className="hover:bg-accent/50 transition">
                  <td className="px-4 py-3 font-medium">
                    <Link to="/restaurant/suppliers/$supplierId" params={{ supplierId: s.id }} className="hover:text-primary">{s.name}</Link>
                  </td>
