...
function RevenuePage() {
  ...
  const data = useMemo(() => period === "7" ? revenueChart.slice(-4) : period === "30" ? revenueChart : revenueChart.concat(revenueChart.slice(0, 10)), [period]);
  ...
  const exportCsv = () => {
    ...
    transactions.forEach((t) => {
      ...
      rows.push([t.date, t.orderRef, r?.name || "", String(t.gross), String(t.commission), String(t.net), t.method, t.status]);
    });
    const csv = rows.map((r) => r.join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = "revenus-diambar.csv"; a.click(); URL.revokeObjectURL(url);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <KpiCard icon={ShoppingCart} label="Commandes payées" value={String(orders)} tone="violet" />
        ...
        <div className="h-72">
          <ResponsiveContainer>
            <AreaChart data={data}>
              ...
              <Area type="monotone" dataKey="revenue" stroke="oklch(0.7 0.17 155)" strokeWidth={2.5} fill="url(#rev2)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="glass rounded-2xl overflow-hidden">
        <div className="p-5 border-b border-border"><h3 className="font-semibold">Transactions</h3></div>
        <Table>
          <TableHeader>
            <TableRow>
            ...
            {transactions.map((t) => {
              ...
              return (
                <TableRow key={t.id} className="cursor-pointer hover:bg-accent/50" onClick={() => { window.location.assign(`/farmer/revenue/${t.id}`); }}>
                  ...
                  <TableCell className="text-right text-sm text-muted-foreground">-{formatFCFA(t.commission)}</TableCell>
                  <TableCell className="text-right text-sm font-semibold text-primary">{formatFCFA(t.net)}</TableCell>
                  <TableCell className="text-sm">{t.method}</TableCell>
                  <TableCell>
                    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-semibold ${
                      t.status === "Payé" ? "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border-emerald-500/30" :
                      t.status === "En attente" ? "bg-amber-500/15 text-amber-600 dark:text-amber-400 border-amber-500/30" :
