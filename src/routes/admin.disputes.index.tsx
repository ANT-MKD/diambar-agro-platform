...
export const Route = createFileRoute("/admin/disputes/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Centre de résolution : SLA, taux de litige par partie, montants remboursés.",
      },
    ...
    tab === "all"
      ...
      : tab === "overdue"
        ? stats.overdue > 0 &&
          d.status !== "resolved" &&
          d.status !== "rejected" &&
          new Date(d.slaDueAt).getTime() < Date.now()
        : d.status === tab,
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          {
            l: "Avoirs émis",
            v: `${credits.length} · ${formatFCFA(credits.reduce((s, c) => s + c.amount, 0))}`,
          },
        ].map((k) => (
          ...
          {stats.byParty.map((p) => (
            ...
            >
              <span className="min-w-40 flex-1 font-medium">
                {p.name}{" "}
                <span className="text-[11px] text-muted-foreground">({PARTY_LABEL[p.role]})</span>
              </span>
              ...
              <span
                className={`rounded-full border px-2 py-0.5 text-[11px] font-semibold ${p.count >= 2 ? "border-destructive/30 bg-destructive/10 text-destructive" : "border-border text-muted-foreground"}`}
              >
          ...
          >
            <div className="min-w-52 flex-1">
              <div className="font-semibold">
                {d.reference} · {DISPUTE_CATEGORIES[d.category]?.label} — {d.subcategory}
              </div>
              <div className="text-[11px] text-muted-foreground">
                {d.openedByName} vs {d.againstName} · commande {d.orderRef} ·{" "}
                {d.assignee ?? "non assigné"} · {relativeTime(d.openedAt)}
              </div>
            </div>
            <SlaBadge dispute={d} />
