...
export const Route = createFileRoute("/driver/missions/$missionId/dispute")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Déclarer un incident de course : client absent, colis refusé, adresse erronée." },
...
function DriverOpenDispute() {
  ...
  if (!mission) return <p className="py-12 text-center text-muted-foreground">Mission introuvable</p>;

  return (
    <div className="max-w-2xl space-y-6">
      <PageHeader title="Signaler un incident de course" subtitle={`${mission.reference} · commande ${mission.orderRef}`} actions={
        <Button variant="outline" className="gap-2" onClick={() => navigate({ to: "/driver/missions/$missionId", params: { missionId } })}><ArrowLeft className="h-4 w-4" />Retour</Button>
      } />
      <DisputeForm
        openedByRole="driver"
        ...
        onCreated={(id) => navigate({ to: "/driver/disputes/$disputeId", params: { disputeId: id } })}
      />
    </div>
  );
}
