...
function MessagesPage() {
  ...
  const filtered = convs.filter((c) => {
    ...
    return r?.name.toLowerCase().includes(q.toLowerCase()) || c.lastMessage.toLowerCase().includes(q.toLowerCase());
  });

  const send = () => {
    if (!active || !draft.trim()) return;
    const newMsg = { id: `m${Date.now()}`, from: "me" as const, text: draft.trim(), at: new Date().toISOString() };
    setConvs((arr) => arr.map((c) => c.id === active.id ? { ...c, messages: [...c.messages, newMsg], lastMessage: newMsg.text, lastAt: newMsg.at } : c));
    setDraft("");
  };
        ...
        <div className="border-r border-border flex flex-col">
          ...
          <div className="flex-1 overflow-auto">
            {filtered.map((c) => {
              ...
              return (
                <button key={c.id} onClick={() => openConv(c.id)} className={`w-full p-3 flex items-center gap-3 border-b border-border text-left hover:bg-accent transition ${isActive ? "bg-accent" : ""}`}>
