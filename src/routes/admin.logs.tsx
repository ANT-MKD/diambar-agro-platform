      ...
      <div className="glass rounded-2xl p-1.5 inline-flex gap-1">
        {(["all", "info", "warning", "critical"] as const).map((l) => (
          <button
            key={l}
            onClick={() => setLevel(l)}
            className={`px-3 h-9 rounded-xl text-sm font-medium transition ${level === l ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent"}`}
          >
      ...
      <div className="glass rounded-2xl divide-y divide-border">
        {rows.map((l) => (
          <div key={l.id} className="flex items-center gap-3 px-4 py-3 text-sm">
            <span
              className={`h-2 w-2 rounded-full shrink-0 ${l.level === "critical" ? "bg-destructive" : l.level === "warning" ? "bg-amber-500" : "bg-emerald-500"}`}
            />
