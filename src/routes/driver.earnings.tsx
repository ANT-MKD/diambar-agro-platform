...
function EarningsPage() {
  ...
  const items = useMemo(() => driverEarnings.filter((e) => method === "all" || e.method === method), [method]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Kpi label="Solde disponible" value={formatFCFA(driverProfile.balance)} icon={Wallet} tone="text-primary" />
        <Kpi label="Gains période" value={formatFCFA(total)} icon={TrendingUp} tone="text-emerald-600 dark:text-emerald-400" delta="+18% vs période précédente" />
        <Kpi label="Bonus cumulés" value={formatFCFA(totalBonus)} icon={ArrowUpRight} tone="text-amber-600 dark:text-amber-400" />
        <Kpi label="Missions payées" value={String(paidCount)} icon={Wallet} tone="text-blue-600 dark:text-blue-400" />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <Tabs value={period} onValueChange={(v) => setPeriod(v as typeof period)}>
              <TabsList className="h-8">
                <TabsTrigger value="7d" className="text-xs h-6">7j</TabsTrigger>
                <TabsTrigger value="30d" className="text-xs h-6">30j</TabsTrigger>
                <TabsTrigger value="90d" className="text-xs h-6">90j</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
          <div className="flex items-end gap-3 h-52">
            {driverEarningsChart.map((d) => (
              <div key={d.day} className="flex-1 flex flex-col items-center gap-2">
                <div className="text-[10px] font-semibold text-muted-foreground">{formatFCFA(d.amount).replace(" FCFA", "")}</div>
                <div className="w-full rounded-t-lg bg-gradient-to-t from-primary via-primary/70 to-primary/30 hover:brightness-110 transition" style={{ height: `${(d.amount / max) * 100}%` }} />
        ...
        <div className="glass rounded-2xl p-5">
          ...
          <div className="space-y-3 text-sm">
            <RowStat label="Gains bruts" value={formatFCFA(items.reduce((s, e) => s + e.gross, 0))} />
            <RowStat label="Bonus" value={`+ ${formatFCFA(totalBonus)}`} tone="text-emerald-600" />
            <RowStat label="Commission plateforme (5%)" value={`- ${formatFCFA(totalFees)}`} tone="text-rose-500" />
            <div className="border-t border-border pt-3">
              <RowStat label="Net versé" value={formatFCFA(total)} bold />
            </div>
          </div>
          <div className="mt-4 pt-3 border-t border-border">
            <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold mb-2">Prochain versement</div>
            <div className="rounded-xl bg-primary/5 border border-primary/20 p-3">
              <div className="text-xs text-muted-foreground">Programmé demain</div>
              <div className="font-display text-xl font-bold text-primary mt-1">{formatFCFA(8575)}</div>
            ...
            <tbody>
              {items.map((e) => (
                <tr key={e.id} className="border-t border-border hover:bg-accent/30">
                  ...
                  <td className="p-3 text-right text-emerald-600 dark:text-emerald-400">{e.bonus ? `+${formatFCFA(e.bonus)}` : "—"}</td>
                  ...
                  <td className="p-3">
                    <span className={`text-[10px] font-semibold rounded-full px-2 py-0.5 ${
                      e.status === "Payé" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" :
                      e.status === "Programmé" ? "bg-blue-500/10 text-blue-600 dark:text-blue-400" :
      ...
      <p className="text-xs text-muted-foreground text-center">
        Les paiements sont versés sous 24h sur votre méthode préférée · <Link to="/driver/settings" className="text-primary hover:underline">Modifier</Link>
      </p>
    </div>
  );
}

function Kpi({ label, value, icon: Icon, tone, delta }: { label: string; value: string; icon: typeof Wallet; tone: string; delta?: string }) {
  return (
    <div className="glass rounded-2xl p-4">
      ...
      {delta && <div className="mt-1 text-[10px] text-emerald-600 dark:text-emerald-400">{delta}</div>}
    </div>
  );
}

function RowStat({ label, value, tone, bold }: { label: string; value: string; tone?: string; bold?: boolean }) {
  return (
    <div className="flex items-center justify-between">
