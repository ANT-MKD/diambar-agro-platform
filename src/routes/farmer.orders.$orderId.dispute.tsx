...
export const Route = createFileRoute("/farmer/orders/$orderId/dispute")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Ouvrir un dossier de litige officiel sur une commande producteur." },
    { property: "og:title", content: "Ouvrir un litige — Producteur" },
    { property: "og:description", content: "Ouvrir un dossier de litige officiel sur une commande." },
...
function FarmerOpenDispute() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <PageHeader title="Ouvrir un litige" subtitle={`${order.reference} · ${r?.name ?? "Restaurant"}`} actions={
        <Button variant="outline" className="gap-2" onClick={() => navigate({ to: "/farmer/orders/$orderId", params: { orderId } })}><ArrowLeft className="h-4 w-4" />Retour</Button>
      } />
      <DisputeForm
        openedByRole="farmer"
        ...
        onCreated={(id) => navigate({ to: "/farmer/disputes/$disputeId", params: { disputeId: id } })}
      />
    </div>
  );
}
