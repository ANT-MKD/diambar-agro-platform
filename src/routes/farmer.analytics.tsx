...
function AnalyticsPage() {
  ...
  const heatmap = days.map((d) => ({
    day: d,
    values: hours.map(() => Math.floor(Math.random() * 9)),
  }));
  ...
  const predictions = Array.from({ length: 7 }, (_, i) => ({
    ...
    forecast: Math.max(
      0,
      Math.round(last[last.length - 1].revenue + slope * (i + 1) * 0.9 + Math.sin(i) * 4000),
    ),
  }));
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              onClick={() =>
                downloadCsv(
                  "previsions-revenus",
                  ["Jour", "Revenu prévu FCFA"],
                  predictions.map((p) => [p.day, p.forecast]),
                )
              }
            >
              <Download className="h-4 w-4" />
        ...
        <div className="glass rounded-2xl p-6 lg:col-span-2">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold">Prédiction des revenus (7 jours)</h3>
            </div>
            <span className="text-xs text-muted-foreground">
              Modèle linéaire · niveau de confiance 78%
            </span>
          </div>
          <div className="text-xs text-muted-foreground mb-4">
            Projection basée sur les 7 derniers jours · prévision totale :{" "}
            <b className="text-primary">{formatFCFA(totalForecast)}</b>
          </div>
          <div className="h-64">
            <ResponsiveContainer>
              <AreaChart
                data={[
                  ...last.map((x) => ({ day: x.day, real: x.revenue, forecast: undefined })),
                  ...predictions.map((x) => ({
                    day: x.day,
                    real: undefined,
                    forecast: x.forecast,
              ...
              >
                <defs>
                  ...
                  <linearGradient id="forecast" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="oklch(0.7 0.2 260)" stopOpacity={0.45} />
      ...
      <div className="glass rounded-2xl p-6">
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Insights basés sur vos données des 30 derniers jours
        </p>
        <div className="grid md:grid-cols-3 gap-3">
          {[
            {
              tone: "emerald",
              title: "Augmentez le stock de Poulet fermier",
              body: "+22 commandes ce mois, ruptures probables d'ici 4 jours.",
            },
            {
              tone: "amber",
              title: "Lancez une promo sur les Mangues Kent",
              body: "0 vente cette semaine — proposez -15% pour relancer la demande.",
            },
            {
              tone: "blue",
              title: "Saint-Louis est sous-exploité",
              body: "Seulement 3 commandes — contactez 5 restaurants locaux pour ouvrir le marché.",
            },
          ].map((tip, i) => (
            <div
              key={i}
              className={`rounded-xl border p-4 ${tip.tone === "emerald" ? "border-emerald-500/30 bg-emerald-500/5" : tip.tone === "amber" ? "border-amber-500/30 bg-amber-500/5" : "border-blue-500/30 bg-blue-500/5"}`}
            >
          ...
          <table className="w-full text-xs">
            <thead>
              <tr>
                <th></th>
                {hours.map((h) => (
                  <th key={h} className="font-medium text-muted-foreground pb-2">
                    {h}
                  </th>
                ))}
              </tr>
                ...
                <tr key={row.day}>
                  ...
                  {row.values.map((v, i) => (
                    <td key={i} className="p-1">
                      <div
                        className="aspect-square rounded-md grid place-items-center font-semibold text-foreground"
                        style={{
                          background: `color-mix(in oklab, oklch(0.7 0.17 155) ${v * 11}%, transparent)`,
                        }}
                      >
                        {v}
                      </div>
