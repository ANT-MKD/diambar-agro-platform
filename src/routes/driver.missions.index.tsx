...
function MissionsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Missions"
        subtitle={`${count("available")} disponible(s) · ${count("active")} en cours`}
      />
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-muted-foreground mt-0.5">
                      {m.orderRef} · {f?.farm} → {r?.name}
                    </div>
                  </div>
                </div>

                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                  <Info icon={MapPin} label="Trajet">
                    {m.pickup.city} → {m.dropoff.city}
                  </Info>
                  <Info icon={Clock} label="Durée">
                    ~ {m.estimatedMinutes} min
                  </Info>
                  <Info icon={Package} label="Poids">
                    {m.weightKg} kg
                  </Info>
                  <Info icon={Clock} label="Programmée">
