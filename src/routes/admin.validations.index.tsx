...
export const Route = createFileRoute("/admin/validations/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "File de validation des agriculteurs et livreurs : documents, identité, décision.",
      },
...
function AdminValidations() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Validations"
        subtitle="Contrôle des documents avant activation d'un compte"
      />
      <div className="glass rounded-2xl p-1.5 inline-flex gap-1">
        {(["pending", "approved", "rejected"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 h-9 rounded-xl text-sm font-medium transition ${tab === t ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent"}`}
          >
            {t === "pending" ? "En attente" : t === "approved" ? "Approuvées" : "Rejetées"}
            <span className="ml-2 text-[11px] opacity-80">
              {validations.filter((v) => v.status === t).length}
            </span>
          </button>
        ))}
      </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-muted-foreground">
                    Déposé {relativeTime(v.submittedAt)}
                  </div>
                </div>
