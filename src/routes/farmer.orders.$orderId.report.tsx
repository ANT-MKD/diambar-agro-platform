...
function ReportPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <PageHeader title="Signaler un problème" subtitle={`${order.reference} · ${r?.name}`} actions={
      ...
      <div className="glass rounded-2xl p-6 space-y-5">
        <div className="flex items-start gap-3 rounded-xl bg-amber-500/10 border border-amber-500/30 p-4">
          ...
          <div className="text-sm">
            <div className="font-semibold text-amber-500">Support Diambar prendra contact sous 4h</div>
            <div className="text-muted-foreground mt-0.5">Commande : <b>{formatFCFA(order.total)}</b></div>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <Select value={issue} onValueChange={setIssue}>
            ...
            <SelectContent>{ISSUES.map((i) => <SelectItem key={i.v} value={i.v}>{i.label}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <Textarea rows={5} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Décrivez ce qui s'est passé, l'heure, les personnes impliquées…" />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <label className="flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-border p-6 cursor-pointer hover:border-primary/50 transition">
            ...
            <span className="text-xs text-muted-foreground">Glissez vos fichiers ou cliquez (JPG, PNG, PDF · max 5 Mo)</span>
