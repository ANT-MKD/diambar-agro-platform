...
const TYPES = [
  { v: "in", label: "Entrée (récolte / approvisionnement)", icon: Plus, tone: "bg-emerald-500/10 text-emerald-500 border-emerald-500/30" },
  { v: "out", label: "Sortie (perte / casse / périmé)", icon: Minus, tone: "bg-rose-500/10 text-rose-500 border-rose-500/30" },
  { v: "adjust", label: "Inventaire (saisie absolue)", icon: ClipboardEdit, tone: "bg-blue-500/10 text-blue-500 border-blue-500/30" },
] as const;
...
function NewMovementPage() {
  ...
  const next = !product ? 0 : type === "in" ? product.stock + qty : type === "out" ? Math.max(0, product.stock - qty) : qty;

  const submit = () => {
    if (!product || qty <= 0) { toast.error("Choisissez un produit et une quantité > 0"); return; }
    movementActions.create({ productId: product.id, type, qty, reason: reason || "—", operator: "Mamadou" });
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <PageHeader title="Nouveau mouvement de stock" subtitle="Enregistrez une entrée, sortie ou inventaire" actions={
      ...
      <div className="glass rounded-2xl p-6 space-y-5">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <Select value={productId} onValueChange={setProductId}>
              ...
              <SelectContent>{products.map((p) => <SelectItem key={p.id} value={p.id}>{p.name} ({p.stock} {p.unit})</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          <Label>Motif</Label>
          <Textarea rows={3} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Récolte du matin, perte au transport, inventaire mensuel…" />
        </div>
        ...
        {product && (
          <div className="rounded-xl bg-primary/10 border border-primary/20 p-4 flex items-center justify-between">
            <div>
              ...
              <div className="font-bold text-lg">{product.stock} → <span className="text-primary">{next}</span> {product.unit}</div>
            </div>
          </div>
        )}

        <div className="flex justify-end gap-2">
          <Button variant="outline" onClick={() => navigate({ to: "/farmer/stock" })}>Annuler</Button>
