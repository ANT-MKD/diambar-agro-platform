        ...
        <div className="flex items-center gap-1 text-xs">
          {(["all", "favorites", "suspended"] as const).map((s) => (
            <button
              key={s}
              onClick={() => setTab(s)}
              className={`px-3 py-1.5 rounded-lg font-medium transition ${tab === s ? "bg-primary text-primary-foreground" : "hover:bg-accent text-muted-foreground"}`}
            >
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        {filtered.map((s) => {
          ...
          return (
            ...
            >
              {s.suspended && (
                <span className="absolute top-3 right-3 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-destructive/10 text-destructive">
                  SUSPENDU
                </span>
              )}
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {s.favorite && <Heart className="h-3.5 w-3.5 fill-rose-500 text-rose-500" />}
                  </div>
              ...
              <div className="flex gap-2 pt-1">
                ...
                <Button asChild variant="outline" size="sm">
                  <Link to="/restaurant/suppliers/$supplierId/edit" params={{ supplierId: s.id }}>
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => supplierActions.toggleSuspend(s.id)}
                >
                  {s.suspended ? (
