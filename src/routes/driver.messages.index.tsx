...
export const Route = createFileRoute("/driver/messages/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Toutes vos conversations avec les restaurants et producteurs." },
...
function DriverMessagesList() {
  ...
  const filtered = enriched.filter(
    (c) => c.resto.name.toLowerCase().includes(q.toLowerCase()) || c.lastMessage.toLowerCase().includes(q.toLowerCase()),
  );
  const unread = convs.reduce((s, c) => s + c.unread, 0);

  return (
    <div className="space-y-6">
      <PageHeader title="Messages" subtitle={`${convs.length} conversation(s) · ${unread} non lue(s)`} />

      <div className="glass rounded-2xl p-3">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Rechercher un restaurant, un message…" className="pl-9" />
        </div>
      </div>

      {filtered.length === 0 ? (
        <EmptyState icon={MessageSquare} title="Aucune conversation" description="Vos échanges avec les restaurants apparaîtront ici." />
      ) : (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  {c.unread > 0 && (
                    <span className="grid h-5 min-w-5 px-1 place-items-center rounded-full bg-primary text-primary-foreground text-[10px] font-bold">{c.unread}</span>
                  )}
                </div>
              </div>
