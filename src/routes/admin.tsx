  ...
  const navSections = [
    {
      ...
      items: [
        ...
        { to: "/admin/validations", label: "Validations", icon: ShieldCheck, badge: pendingValidations },
    ...
    <div className="min-h-screen bg-background flex">
      <aside className="hidden lg:flex w-64 shrink-0 flex-col border-r border-border bg-sidebar">
              ...
              {section.items.map((it) => {
                ...
                return (
                  <Link key={it.to} to={it.to} className={`group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition ${active ? "bg-primary text-primary-foreground shadow-lg shadow-primary/20" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`}>
                    ...
                    {it.badge > 0 && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-destructive text-destructive-foreground">{it.badge}</span>}
                  </Link>
                );
              })}
            </div>
        ...
        <div className="p-3 border-t border-border">
          <div className="glass rounded-xl p-3 flex items-center gap-3">
            <img src="https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=80" alt="" className="h-9 w-9 rounded-full object-cover" />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-30 glass-strong border-b border-border flex items-center gap-3 px-4 lg:px-6 h-14">
          ...
          <ThemeToggle />
          <Link to="/admin/validations" className="grid h-9 w-9 place-items-center rounded-xl hover:bg-accent relative">
            <Bell className="h-4 w-4" />
            {pendingValidations + openDisputes > 0 && (
              <span className="absolute -top-0.5 -right-0.5 h-4 min-w-4 px-1 text-[9px] font-bold rounded-full bg-destructive text-destructive-foreground grid place-items-center">{pendingValidations + openDisputes}</span>
            )}
          </Link>
          <img src="https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=80" alt="" className="h-9 w-9 rounded-full object-cover ring-2 ring-primary/40" />
        </header>
