...
export const Route = createFileRoute("/farmer/messages/$conversationId")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Échangez en direct avec un restaurant client sur Diambar Agro." },
    ...
    return (
      <div className="space-y-6">
        ...
        <EmptyState
          ...
          action={<Button onClick={() => navigate({ to: "/farmer/messages" })}>Retour aux messages</Button>}
        />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link to="/farmer/messages" className="grid h-9 w-9 place-items-center rounded-xl border border-border hover:bg-accent">
          <ArrowLeft className="h-4 w-4" />
        </Link>
        <PageHeader title={r?.name ?? "Conversation"} subtitle={`${r?.city ?? ""} · ${r?.type ?? "Restaurant"}`} />
      </div>
      ...
      <div className="glass rounded-2xl overflow-hidden flex flex-col h-[640px]">
        <div className="p-4 border-b border-border flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-muted-foreground">En ligne · répond en général en 10 min</div>
          </div>
          <Button variant="outline" size="icon"><Phone className="h-4 w-4" /></Button>
          <Link to="/farmer/orders"><Button variant="outline" size="sm">Voir ses commandes</Button></Link>
        </div>
