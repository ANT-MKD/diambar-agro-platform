...
function InvoicesList() {
  ...
  const paidOrders = useMemo(
    () => orders.filter((o) => o.status === "delivered" || o.status === "delivering"),
    [orders],
  );
  ...
  }, [orders, paidOrders, q, status, period, nowYear]);
      ...
      {overdue.length > 0 && (
        <div className="glass rounded-2xl p-4 flex items-center gap-3 border border-amber-500/30 bg-amber-500/5">
          ...
          <Button
            ...
            variant="outline"
            className="gap-1"
            onClick={() => toast.success(`Relance envoyée pour ${overdue.length} facture(s)`)}
          >
      ...
      <div className="glass rounded-2xl p-4 flex flex-wrap items-center gap-3">
        <div className="flex-1 min-w-[220px] relative">
          ...
          <Input
            ...
            placeholder="Rechercher une facture ou une commande…"
            className="pl-9"
          />
        </div>
        <select
        ...
        <div className="flex items-center gap-1 text-xs">
          ...
          {(["all", "paid", "pending"] as const).map((s) => (
            <button
              key={s}
              onClick={() => setStatus(s)}
              className={`px-3 py-1.5 rounded-lg font-medium transition ${status === s ? "bg-primary text-primary-foreground" : "hover:bg-accent text-muted-foreground"}`}
            >
      ...
      <div className="glass rounded-2xl overflow-hidden">
        ...
        {filtered.map((o) => {
          ...
          return (
            <div
              key={o.id}
              className="grid grid-cols-[1fr_1.2fr_1fr_1fr_120px_140px] items-center px-4 py-3 border-b border-border last:border-0 hover:bg-accent/40 transition text-sm"
            >
              <Link
                to="/restaurant/invoices/$invoiceId"
                params={{ invoiceId: o.id }}
                className="font-mono font-semibold text-primary inline-flex items-center gap-2"
              >
                {invoiceNumberFor(o.id)}
                {isOverdue(o) && (
                  <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30">
                    Retard
                  </span>
                )}
              </Link>
              ...
              <span
                className={`text-[11px] font-semibold px-2 py-0.5 rounded-full w-fit ${paid ? "bg-emerald-500/10 text-emerald-500" : "bg-amber-500/10 text-amber-500"}`}
              >
                {paid ? "Payée" : "En attente"}
              </span>
              <div className="flex items-center justify-end gap-1">
                <Button asChild variant="ghost" size="sm">
                  <Link to="/restaurant/invoices/$invoiceId" params={{ invoiceId: o.id }}>
                    ...
                    const data: InvoiceData = {
                      ...
                      items: o.items.map((it) => {
                        ...
                        return {
                          ...
                          sub: `Commande ${o.reference} · ${f?.name ?? ""}`,
                          qty: String(it.qty),
                          qtyUnit: p?.unit,
                          unitPrice: it.price,
