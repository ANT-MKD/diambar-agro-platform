...
export const Route = createFileRoute("/farmer/settings/payments")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Configurez vos comptes Wave, Orange Money et Free Money pour recevoir vos paiements." },
...
function PaymentSettings() {
  const [form, setForm] = useState({ wave: "77 123 45 67", orange: "78 200 33 44", free: "", primary: "wave", threshold: "25000" });
  return (
    <SettingsCard title="Méthodes de paiement" description="Comptes mobile money pour recevoir vos paiements.">
      ...
      <FieldRow label="Seuil de retrait automatique (FCFA)" hint="Un retrait est proposé dès que votre solde dépasse ce montant.">
        <Input type="number" step="1000" value={form.threshold} onChange={(e) => setForm({ ...form, threshold: e.target.value })} />
      </FieldRow>
    </SettingsCard>
  );
}