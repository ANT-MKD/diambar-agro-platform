        ...
        >
          <div className="glass rounded-2xl p-5 flex flex-col">
            <div className="flex items-center justify-between">
              ...
              <ProgressCircle
                value={suppliers.filter((s) => s.favorite).length}
                max={suppliers.length}
      ...
      <div className="grid lg:grid-cols-3 gap-4">
          ...
          {active.map((o) => {
            ...
            return (
              <Link
                ...
                params={{ orderId: o.id }}
                className="flex items-center gap-3 p-3 rounded-xl border border-border hover:bg-accent/30"
              >
                <img src={farmer?.avatar} alt="" className="h-10 w-10 rounded-xl object-cover" />
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-sm truncate">
                    {o.reference} · {farmer?.farm}
                  </div>
                  <div className="text-[11px] text-muted-foreground">
                    {relativeTime(o.createdAt)} · {o.items.length} article(s)
                  </div>
                </div>
        ...
        <div className="glass rounded-2xl p-5 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display text-lg font-bold">Alertes stock fournisseurs</h2>
          </div>
