    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-2">
        {[1, 2, 3].map((s) => (
          <div key={s} className="flex-1 flex items-center gap-2">
            <div className={`h-8 w-8 rounded-full grid place-items-center text-xs font-bold shrink-0 ${step >= s ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>{step > s ? <Check className="h-4 w-4" /> : s}</div>
            <span className={`text-xs font-medium ${step >= s ? "" : "text-muted-foreground"}`}>{s === 1 ? "Livraison" : s === 2 ? "Paiement" : "Confirmation"}</span>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass rounded-2xl p-6 space-y-4">
          ...
          {step === 2 && (
            <>
              <h3 className="font-display text-lg font-bold flex items-center gap-2"><CreditCard className="h-5 w-5 text-primary" />Méthode de paiement</h3>
              <div className="grid sm:grid-cols-2 gap-2">
                {PAY.map((m) => (
                  <button key={m} onClick={() => setMethod(m)} className={`p-4 rounded-xl border text-left transition ${method === m ? "border-primary bg-primary/5" : "border-border hover:bg-accent/30"}`}>
                    <div className="font-semibold text-sm">{m}</div>
                    <div className="text-[11px] text-muted-foreground mt-0.5">{m === "Espèces" ? "À la livraison" : "Paiement instantané"}</div>
                  </button>
                ))}
              </div>
            </>
        ...
        <div className="glass rounded-2xl p-5 h-fit space-y-3">
          ...
          <div className="space-y-2 text-sm max-h-48 overflow-auto">
            {lines.map((l) => (
              <div key={l.productId} className="flex justify-between gap-2">
                <span className="text-muted-foreground truncate">{l.product.name} ×{l.qty}</span>
          ...
          <div className="border-t border-border pt-2 space-y-1.5 text-sm">
            ...
            <div className="flex justify-between"><span className="text-muted-foreground">Livraison</span><span>{formatFCFA(delivery)}</span></div>
            <div className="flex justify-between font-bold text-lg pt-1"><span>Total</span><span className="text-primary">{formatFCFA(total)}</span></div>
          </div>
