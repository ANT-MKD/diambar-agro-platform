  ...
  head: () => ({
    meta: [
      { title: "Blog — Agriculture, restauration et logistique | Diambar Agro" },
      {
        name: "description",
        content:
          "Analyses terrain sur les pertes post-récolte, le coût matière en restauration et la logistique du dernier kilomètre au Sénégal.",
      },
      { property: "og:title", content: "Blog — Diambar Agro" },
      {
        property: "og:description",
        content: "Analyses terrain sur l'agriculture, la restauration et la logistique au Sénégal.",
      },
...
function BlogIndex() {
  ...
  return (
    <PublicShell>
      <PageHero
        eyebrow="Blog"
        title="Le terrain, en clair"
        subtitle="Ce que nous apprenons des producteurs, des cuisines et des routes du Sénégal."
      />
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {categories.map((c) => (
              <button
                key={c}
                onClick={() => setCat(c)}
                className={`rounded-xl px-4 py-2 text-sm font-medium transition ${cat === c ? "bg-primary text-primary-foreground" : "glass hover:bg-accent"}`}
              >
                {c}
              </button>
            ))}
          ...
          <div className="mt-10 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {posts.map((p) => (
              <Link
                ...
                to="/blog/$slug"
                params={{ slug: p.slug }}
                className="glass rounded-3xl overflow-hidden hover:scale-[1.02] hover:border-emerald-500/30 transition-all"
              >
                <img
                  src={p.cover}
                  alt={p.title}
