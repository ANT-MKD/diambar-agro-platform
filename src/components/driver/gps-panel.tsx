...
export function GpsPanel({
  ...
  const etaClock = new Date(Date.now() + etaMin * 60000).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" });
  ...
  return (
    <div className="glass rounded-2xl overflow-hidden">
      <div className="p-4 flex items-center justify-between gap-3 border-b border-border">
        <div className="flex items-center gap-2">
          <div className="relative">
            <span className={`block h-2 w-2 rounded-full ${live ? "bg-emerald-500" : "bg-muted-foreground"}`} />
            {live && <span className="absolute inset-0 h-2 w-2 rounded-full bg-emerald-500 animate-ping" />}
          </div>
          <span className="text-sm font-semibold">{live ? "GPS actif · position transmise" : "GPS en veille"}</span>
        </div>
        <label className="flex items-center gap-2 text-[11px] text-muted-foreground">
          <Share2 className="h-3.5 w-3.5" />Partager au client
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 px-4 pb-4">
        <Metric icon={Satellite} label="Coordonnées" value={`${lat.toFixed(4)}, ${lng.toFixed(4)}`} />
...
function Metric({ icon: Icon, label, value }: { icon: typeof Navigation; label: string; value: string }) {
  return (
