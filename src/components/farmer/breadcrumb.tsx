  ...
  return (
    <nav className="hidden md:flex items-center text-xs text-muted-foreground gap-1.5">
      ...
      {parts.map((p, i) => {
        ...
        const label = LABELS[p] ?? (p.startsWith("$") || /^[a-z0-9]{1,5}\d/.test(p) ? "#" + p.slice(0, 6) : p);
        return (
          <span key={to} className="flex items-center gap-1.5">
            <ChevronRight className="h-3 w-3 opacity-50" />
            {isLast ? <span className="text-foreground font-medium">{label}</span> : <Link to={to} className="hover:text-foreground">{label}</Link>}
          </span>
        );
      })}
    </nav>
