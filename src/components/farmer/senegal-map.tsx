...
export function SenegalMap() {
  ...
  return (
    <div className="relative w-full aspect-[4/3] rounded-xl border border-border overflow-hidden">
          ...
          >
            <div className="relative grid place-items-center">
              <div
                ...
                style={{ width: size, height: size, background: `${p.color}33` }}
              />
              <div
                className="h-8 w-8 rounded-full grid place-items-center text-white text-[10px] font-bold shadow-lg ring-2 ring-white/30"
                style={{ background: p.color }}
              >
                <MapPin className="h-3.5 w-3.5" />
              </div>
              <span className="absolute top-9 whitespace-nowrap text-[10px] font-medium glass-strong rounded px-1.5 py-0.5">
                {p.region} · {p.count}
              </span>
            </div>
          </div>
      ...
      {/* Attribution */}
      <div className="absolute left-3 bottom-2 text-[9px] text-white/60 font-medium">
        Sénégal · Vue commandes
      </div>
    </div>
  );
}
