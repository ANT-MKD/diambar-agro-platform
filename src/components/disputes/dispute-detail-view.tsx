    ...
    <div className="space-y-6">
      ...
      {!closed && sla.overdue && (
        <div className="flex items-center gap-3 rounded-2xl border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">
          ...
          <span>
            Délai de réponse dépassé ({sla.label}) — escalade niveau {d.escalations + 1}{" "}
            recommandée.
          </span>
        </div>
      )}
          ...
          <div className="glass rounded-2xl p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              ...
              <Field
                ...
                value={
                  d.grantedAmount === null ? "En attente de décision" : formatFCFA(d.grantedAmount)
                }
              />
              <Field
                label="Partie responsable"
                value={d.liableParty ? PARTY_LABEL[d.liableParty] : "Non déterminée"}
              />
              <Field
          ...
          {d.decision && (
            <div className="glass rounded-2xl border border-emerald-500/30 p-5">
              ...
              <div className="mt-2 text-sm">
                {outcomeLabel(d.decision.outcome)} · <b>{formatFCFA(d.decision.grantedAmount)}</b> à
                la charge de <b>{PARTY_LABEL[d.decision.debitedParty]}</b>
              </div>
              <p className="mt-1 text-sm text-muted-foreground">{d.decision.reason}</p>
              <div className="mt-2 text-[11px] text-muted-foreground">
                Rendue par {d.decision.by} · {relativeTime(d.decision.at)}
              </div>
            </div>
          )}
        ...
        <div className="space-y-4">
          <div className="glass rounded-2xl p-5">
            ...
            <div className="mt-3 space-y-2 text-sm">
              {links ?? <span className="text-muted-foreground">Aucun élément lié.</span>}
            </div>
          </div>
                ...
                >
                  {a.dataUrl && a.mime.startsWith("image/") ? (
                    ...
                    <span className="grid h-10 w-10 place-items-center rounded-lg bg-muted text-[10px] text-muted-foreground">
                      {a.mime.includes("pdf") ? "PDF" : "IMG"}
                    </span>
                  )}
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-xs font-medium">{a.name}</div>
                    <div className="text-[11px] text-muted-foreground">
                      {(a.size / 1024).toFixed(0)} Ko · {a.by}
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="glass space-y-3 rounded-2xl p-5">
              ...
              <div className="space-y-1.5 border-t border-border pt-3">
                ...
                <Select value={outcome} onValueChange={(v) => setOutcome(v as DisputeOutcome)}>
                  ...
                  <SelectContent>
                    {(
                      ["refund", "partial", "credit", "goodwill", "rejected"] as DisputeOutcome[]
                    ).map((o) => (
              ...
              <div className="space-y-1.5">
                ...
                <Select value={liable} onValueChange={(v) => setLiable(v as DisputeParty)}>
                  ...
                  <SelectContent>
                    {(["farmer", "driver", "restaurant", "platform"] as DisputeParty[]).map((p) => (
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  placeholder="Preuves retenues, règle appliquée…"
                />
              </div>
              <Button
                className="w-full"
                onClick={() => {
                  ...
                  toast.success(
                    amount > 0
                      ? `Décision enregistrée · avoir de ${formatFCFA(amount)} émis`
                      : "Réclamation rejetée",
                  );
                }}
              >
