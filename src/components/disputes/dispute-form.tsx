...
export function DisputeForm(p: Props) {
  ...
  const [subcategory, setSubcategory] = useState(
    DISPUTE_CATEGORIES[p.defaultCategory ?? "quality"].subs[0],
  );
  ...
  const submit = () => {
    if (!description.trim() || description.trim().length < 20) {
      toast.error("Décrivez le problème (20 caractères minimum)");
      return;
    }
    const value = Number(amount) || 0;
    if (value <= 0) {
      toast.error("Indiquez le montant réclamé");
      return;
    }
    if (p.maxAmount && value > p.maxAmount) {
      toast.error(`Le montant ne peut dépasser ${formatFCFA(p.maxAmount)}`);
      return;
    }
    ...
    <div className="glass space-y-5 rounded-2xl p-6">
      <div className="flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
        ...
        <div className="text-sm">
          ...
          <div className="mt-0.5 text-muted-foreground">
            Commande <b>{p.orderRef}</b>
            {p.maxAmount ? (
              <>
                {" "}
                · montant maximum réclamable <b>{formatFCFA(p.maxAmount)}</b>
              </>
            ) : null}{" "}
            · réponse du support sous <b>{slaHours}h</b>.
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <Select
            ...
            onValueChange={(v) => {
              setCategory(v);
              setSubcategory(DISPUTE_CATEGORIES[v].subs[0]);
            }}
          >
            <SelectTrigger>
              <SelectValue />
        ...
        <div className="space-y-1.5">
          ...
          <Select value={subcategory} onValueChange={setSubcategory}>
            ...
            <SelectContent>
              {DISPUTE_CATEGORIES[category].subs.map((s) => (
        ...
        <div className="space-y-1.5">
          ...
          <Select value={againstIdx} onValueChange={setAgainstIdx}>
            ...
            <SelectContent>
              {p.againstOptions.map((o, i) => (
                <SelectItem key={`${o.role}-${o.name}`} value={String(i)}>
                  {PARTY_LABEL[o.role]} · {o.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
      ...
      <div className="space-y-1.5">
        ...
        <Textarea
          ...
          placeholder="Décrivez les faits : heure, quantités, personnes impliquées, échanges déjà eus…"
        />
      </div>
      ...
      <FileDrop
        ...
        by={p.openedByName}
        kind="photo"
        label="Preuves (photos, bon de livraison, capture de paiement)"
      />
