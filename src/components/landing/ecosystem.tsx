...
const profiles = [
  {
    emoji: "👨🏾‍🌾",
    role: "Agriculteurs",
    subtitle: "Vendez plus, sans intermédiaire",
    color: "emerald",
  ...
  },
  {
    emoji: "🍽️",
    role: "Restaurants",
    subtitle: "Approvisionnez-vous mieux, moins cher",
    color: "amber",
  ...
  },
  {
    emoji: "🚚",
    role: "Livreurs",
    subtitle: "Des missions, des revenus, de la liberté",
    color: "blue",
  ...
  },
  {
    ...
    subtitle: "Contrôlez tout l'écosystème",
    color: "violet",
      ...
      <div className="mx-auto max-w-7xl px-4">
        ...
        <div className="mt-14 grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {profiles.map((p) => (
            <div
              key={p.role}
              className={`glass rounded-3xl p-6 ${p.border} ${p.bg} flex flex-col hover:-translate-y-1 transition-transform`}
            >
              ...
              <div
                className={`mt-5 rounded-xl border ${p.border} ${p.bg} p-3 text-center text-sm font-semibold text-${p.color}-500`}
              >
                {p.stat}
              </div>
              <Link
                to={p.href}
                className="mt-4 inline-flex items-center justify-center gap-2 rounded-xl bg-foreground/5 hover:bg-foreground/10 px-4 py-2.5 text-sm font-semibold transition group"
              >
                Rejoindre{" "}
