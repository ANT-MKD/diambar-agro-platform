...
export function SecurityPanel({
  ...
  const toggleTwoFa = (on: boolean) => {
    ...
    setStep("verify");
    toast.info(
      `Code de vérification envoyé (${channel === "sms" ? "SMS" : channel === "email" ? "e-mail" : "application"})`,
      { description: `Code démo : ${generated}` },
    );
  };
    ...
    <>
      <SettingsCard title={title} description={description} onSave={save}>
        ...
        <div className="text-xs text-muted-foreground">
          Dernière modification {relativeTime(sec.passwordUpdatedAt)}.
        </div>
      </SettingsCard>
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Un code à usage unique est demandé à chaque nouvelle connexion.
            </p>
          </div>
        </div>
        ...
        <ToggleRow
          ...
          description={
            sec.twoFa
              ? `Canal : ${sec.twoFaChannel === "sms" ? "SMS" : sec.twoFaChannel === "email" ? "E-mail" : "Application"}`
              : "Activez pour recevoir un code de vérification"
          }
          checked={sec.twoFa}
          ...
          <div className="rounded-xl border border-border p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  variant="outline"
                  onClick={() => {
                    securityActions.regenerateCodes();
                ...
                <Button
                  ...
                  className="gap-1.5"
                  onClick={() => {
                    navigator.clipboard?.writeText(sec.recoveryCodes.join("\n"));
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {sec.recoveryCodes.map((c) => (
                <div
                  key={c}
                  className="rounded-lg bg-muted/50 px-3 py-2 text-center font-mono text-xs"
                >
                  {c}
                </div>
              ))}
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <Button
            ...
            className="gap-1.5"
            onClick={() => {
              securityActions.revokeAllOthers();
              ...
              >
                ...
                {s.current ? (
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      securityActions.revokeSession(s.id);
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        ...
        <div className="space-y-2">
          {sec.logins.slice(0, 8).map((l) => (
            ...
            >
              <span
                className={`h-2 w-2 rounded-full ${l.result === "success" ? "bg-emerald-500" : "bg-destructive"}`}
              />
              <div className="flex-1 min-w-0">
                <div className="font-medium truncate">{l.device}</div>
                <div className="text-xs text-muted-foreground">
                  {l.place} · {l.ip} · {relativeTime(l.at)}
                </div>
              </div>
              <span
                className={`text-[10px] font-semibold rounded-full px-2 py-0.5 ${l.result === "success" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "bg-destructive/10 text-destructive"}`}
              >
