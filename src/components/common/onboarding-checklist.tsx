...
export function OnboardingChecklist({
  ...
  return (
    <div className="glass rounded-2xl p-5 relative">
      <button
        aria-label="Fermer"
        onClick={() => onboardingActions.dismiss(storageKey)}
        className="absolute top-3 right-3 h-7 w-7 grid place-items-center rounded-lg hover:bg-accent text-muted-foreground"
      >
        <X className="h-4 w-4" />
      </button>
      <div className="flex items-center gap-3 pr-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-muted-foreground">
            {pct === 100
              ? "Tout est prêt 🎉"
              : `${total - done} étape(s) restante(s) pour finaliser votre compte.`}
          </p>
        </div>
      </div>
      <div className="mt-4 grid sm:grid-cols-2 gap-2">
        {items.map((it) => {
          ...
          return (
            ...
            >
              <span
                className={`h-5 w-5 rounded-md grid place-items-center shrink-0 border ${isDone ? "bg-primary border-primary text-primary-foreground" : "border-border"}`}
              >
                {isDone && <Check className="h-3.5 w-3.5" />}
              </span>
              <span
                className={`flex-1 truncate ${isDone ? "line-through text-muted-foreground" : ""}`}
              >
                {it.label}
              </span>
            </button>
